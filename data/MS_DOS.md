...
MS-DOS was a renamed form of 86-DOS – owned by Seattle Computer Products, written by Tim Paterson. Development of 86-DOS took only six weeks, as it was basically a clone of Digital Research's CP/M (for 8080/Z80 processors), ported to run on 8086 processors and with two notable differences compared to CP/M: an improved disk sector buffering logic, and the introduction of FAT12 instead of the CP/M filesystem. This first version was shipped in August 1980. Microsoft, which needed an operating system for the IBM Personal Computer, hired Tim Paterson in May 1981 and bought 86-DOS 1.10 for US$25,000 that July. Microsoft kept the version number, but renamed it MS-DOS. They also licensed MS-DOS 1.10/1.14 to IBM, which, in August 1981, offered it as PC DOS 1.0 as one of three operating systems for the IBM 5150 or the IBM PC.
...
On March 25, 2014, Microsoft made partial source code (including sourceless binaries) to SCP MS-DOS 1.25 and a mixture of Altos MS-DOS 2.11 and TeleVideo PC DOS 2.11 available to the public under the Microsoft Research License Agreement, which made the partial sources source-available, but not open source as defined by Open Source Initiative or free software by Free Software Foundation standards. Microsoft would later re-license the partial sources and sourceless binaries under the MIT License on September 28, 2018, making these versions partially open source. Microsoft later released partial source code and sourceless binaries for MS-DOS 4.00 on April 25, 2024, under the same license.
...
PC DOS and MS-DOS's code bases diverged after the IBM PC's debut. Microsoft later discontinued internal MS-DOS development and licensed COMPAQ-DOS from Compaq as MS-DOS.


In the former Eastern bloc, MS-DOS derivatives named DCP (Disk Control Program ) 3.20 and 3.30 (DCP 1700, DCP 3.3) and WDOS existed in the late 1980s. They were produced by the East German electronics manufacturer VEB Robotron.
...
In the business world, the 808x-based machines that MS-DOS was tied to faced competition from the Unix operating system; the latter ran on many different hardware architectures. Microsoft itself sold a version of Unix for the PC called Xenix.
...
The 16-bit versions of Windows (up to 3.11) ran as a graphical user interface (GUI) on top of MS-DOS. With Windows 95, 98, and Me, the role of MS-DOS was reduced to a boot loader according to Microsoft, with MS-DOS programs running in a virtual DOS machine within 32-bit Windows, with ability to boot directly into MS-DOS retained as a backward compatibility option for applications that required real mode access to the hardware, which was generally not possible within Windows. The command line accessed the DOS command line (usually COMMAND.COM) through a Windows module (WINOLDAP.MOD).
...
Due to the historical nature of the software, Microsoft will not accept any pull requests to the code. However, its permissive MIT License allows users to download, modify and fork the software as long as credit is given to its original authors.
